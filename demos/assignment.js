...
function calculateSurfaceReflectionMatrix(reflectionMat, mirrorModelMatrix, surfaceNormal) {
    ...
    reflectionMat[0] = 1 - 2 * plane[0] * plane[0] + 0.1 * Math.sin(pos[0]);
    reflectionMat[4] = -2 * plane[0] * plane[1] + 0.1 * Math.sin(pos[1]);
    reflectionMat[8] = -2 * plane[0] * plane[2];
    reflectionMat[12] = -2 * plane[3] * plane[0];

    reflectionMat[1] = -2 * plane[1] * plane[0] + 0.1 * Math.sin(pos[0]);
    reflectionMat[5] = 1 - 2 * plane[1] * plane[1] + 0.1 * Math.sin(pos[1]);
...
let mirrorDrawCall = app.createDrawCall(mirrorProgram, mirrorArray)
    ...
    .texture("distortionMap", app.createTexture2D(await loadTexture("water.jpg")));
...
function renderReflectionTexture()
{
    ...
    let reflectionMatrix = calculateSurfaceReflectionMatrix(mat4.create(), mirrorModelMatrix, vec3.fromValues(0, 1, 1));
